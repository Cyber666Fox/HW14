...
def movie_by_title(title):
    ...
    db_conect.cur.execute(f"""SELECT title,country,release_year,genre,description FROM netflix WHERE title like '%{title}%' order by release_year desc limmit 1 ;""")
    result = db_conect.cur.fetchone()
    return{"title":result[0], "country": result[1], "release_year": result[2],"genre":result[3],"description":result[4]}
...
def movies_by_ganre(ganre):
    result = execute_query(f"""SELECT title, description FROM netflix WHERE listed_in like '%{ganre}%' order by release_year desc limit 10;""")
    result_list = []
    for movie in result:
...
def search_movie_by_param (movie_type, release_year, ganre):
    query = f"""SELECT title, description FROM netflix WHERE type='{movie_type}'  AND release_year={release_year} AND listed_in LIKE '%{ganre}%'"""
    result = execute_query(query)
    result_list = []
    for movie in result:
        result_list.append({"title":{movie[0]},"description":{movie[1]}})
